...
══════════════════════════════════════════ */
export default function CertCheckout() {
  const navigate  = useNavigate();
  const location  = useLocation();
  const { user }  = useAuth();

  const certState = location.state?.cert || null;
  const cert = certState ? {
    ...certState,
    origPrice: certState.origPrice && certState.origPrice > certState.price
      ? certState.origPrice
      : Math.round((certState.price || 0) * 1.5) || certState.price || 0,
  } : null;
  ...
  useEffect(() => {
    ...
    if (!cert)  { setTimeout(() => navigate('/certifications'), 0); }
  }, [user, cert, navigate]);
  ...
  const handlePay = async () => {
    ...
    const finalCertId = cert?._id || cert?.id || cert?.certId;
    if (!finalCertId) {
      toast.error('Certificate ID missing. Please go back and try again.');
      return;
    }
    ...
    try {
      let enrollData = {};
      try {
        const { data } = await api.post(`/enrollments/${finalCertId}`, {
      ...
      const targetId = String(finalCertId);
      ...
      try {
        const cache = JSON.parse(localStorage.getItem('lf_cert_cache') || '{}');
        cache[targetId] = cert;
        localStorage.setItem('lf_cert_cache', JSON.stringify(cache));
      } catch (cacheErr) {
        console.error('Cache update failed:', cacheErr);
      }

      navigate('/success', {
        state: { cert, certId: finalCertId, isTrial: false },
      });
              ...
              <div className={`${showSummary ? 'block' : 'hidden lg:block'}`}>
                ...
                <div className="px-6 py-5">
                  <div className="flex items-start gap-4">
                    <div className={`w-12 h-12 rounded-xl bg-gradient-to-br ${cert.gradient || 'from-cyan-500 to-blue-600'} flex items-center justify-center text-2xl flex-shrink-0`}>
                      {cert.emoji || '🎓'}
                    </div>
                    <div className="flex-1 min-w-0">
                      <p className="text-sm font-bold text-slate-900 leading-snug">{cert.title}</p>
                      <div className="flex flex-wrap items-center gap-2 mt-1.5">
                        <span className={`text-xs font-mono px-2 py-0.5 rounded-full ${cert.accentBg || 'bg-cyan-50'} ${cert.accentText || 'text-cyan-700'} border ${cert.accentBorder || 'border-cyan-200'}`}>
                          {cert.tag}
                        </span>
                        {cert.level && (
