...
function saveQuizResult(courseId, score, passed) {
  try {
    const all = JSON.parse(localStorage.getItem('lf_course_quiz') || '{}');
    all[courseId] = {
      score, passed,
      date: new Date().toLocaleDateString('en-IN', { day: 'numeric', month: 'short', year: 'numeric' }),
    };
    localStorage.setItem('lf_course_quiz', JSON.stringify(all));
  } catch {}
}
...
export default function Quiz() {
  ...
  function handleSubmit(auto = false) {
    ...
    saveQuizResult(course.id, finalScore, finalScore >= PASS_SCORE);
    setPhase('result');
  }
      ...
      <div className="max-w-lg w-full bg-white border border-slate-200 rounded-2xl overflow-hidden shadow-sm">
        ...
        <div className="p-8">
          <div className="text-center mb-6">
            ...
            <p className="text-slate-500 font-mono text-sm">{course.title}</p>
          </div>
          ...
          <button
            ...
            className={`w-full py-3.5 rounded-xl bg-gradient-to-r ${course.gradient} text-white font-bold text-base transition-all hover:opacity-90 active:scale-[0.98]`}
          >
            Start Quiz
          </button>
          ...
          <div className="flex flex-col gap-3">
            {passed ? (
              <button
                ...
                className={`w-full py-3.5 rounded-xl bg-gradient-to-r ${course.gradient} text-white font-bold text-base transition-all hover:opacity-90`}
              >
    ...
    <div className="min-h-screen bg-slate-50">
      ...
      <div className="bg-white border-b border-slate-200 sticky top-0 z-20">
        ...
        <div className="w-full h-1 bg-slate-100">
          <div className={`h-1 bg-gradient-to-r ${course.gradient} transition-all duration-500`} style={{ width: `${progressPct}%` }} />
        </div>
      </div>
