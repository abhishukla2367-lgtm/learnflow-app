...
function genCourseId(userId, courseId) {
...
export default function CourseCertificate() {
  ...
  usePdfExport(exportPdfRef, courseRef, course?.title?.replace(/\s+/g, '-') || 'certificate', "Official Certificate");
    ...
    return (
      <div className="min-h-screen bg-slate-50 flex items-center justify-center px-4">
        <div className="text-center">
          ...
          <button onClick={() => navigate(`/course-quiz/${course.id}`)} className="px-6 py-3 rounded-xl bg-cyan-600 text-white font-bold text-sm hover:bg-cyan-700 transition-all">
            Take the Quiz
          </button>
        </div>
      </div>
  ...
  const courseIdStr = genCourseId(user?.id || user?.email, course.id);
  ...
  const handleLinkedIn = () => {
    const url = `https://www.linkedin.com/profile/add?startTask=CERTIFICATION_NAME&name=${encodeURIComponent(course.title)}&organizationName=Learnodays&issueYear=${new Date().getFullYear()}&issueMonth=${new Date().getMonth() + 1}&certUrl=${encodeURIComponent(window.location.href)}&courseId=${courseIdStr}`;
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 py-16 print:p-0 print:max-w-none">
        <div className="flex flex-col lg:flex-row justify-start gap-6 items-start print:block lg:pl-14">
          ...
          <div className="w-full lg:w-[700px] flex-shrink-0 print:w-full">
            <div id="certificate" ref={courseRef} className="bg-white overflow-hidden print:overflow-visible shadow-xl print:shadow-none border border-slate-200 print:border-0 print:rounded-none">
  <div className={`h-4 bg-gradient-to-r ${course.gradient}`} style={{ WebkitPrintColorAdjust: 'exact', printColorAdjust: 'exact' }} />
  ...
  <svg viewBox="0 0 200 200" className="w-full h-full" role="img" aria-label="Learnodays">
    ...
    />
                  ...
                  <div className="text-right">
                    ...
                    <p className="text-xs font-bold text-slate-600 font-mono mt-0.5">{courseIdStr}</p>
                  </div>
                </div>
...
<div className="flex justify-center mb-5">
  ...
  >
    <span style={{ color: '#ffffff', fontSize: '1.5rem', lineHeight: 1 }}>{course.emoji}</span>
    <h2 style={{ 
      color: '#ffffff', 
      fontSize: '1.25rem', 
      fontWeight: '900', 
    ...
    >
      {l.title.split('&')[0].trim().split(' ').slice(0, 3).join(' ')}
                ...
                <div className="flex items-end justify-between border-t border-slate-200 pt-10">
                  ...
                  <div className="text-center">
                    <div className={`w-16 h-16 rounded-full bg-gradient-to-br ${course.gradient} flex items-center justify-center mx-auto mb-2 shadow-md`} style={{ WebkitPrintColorAdjust: 'exact', printColorAdjust: 'exact' }}>
          ...
          <div className="w-full lg:w-72 flex-shrink-0 space-y-4 print:hidden">
            <div className="bg-white border border-slate-200 rounded-2xl p-5 shadow-sm">
              ...
              <p className="text-sm text-slate-500 font-mono leading-relaxed">
                You've earned the <span className="text-cyan-600 font-semibold">{course.title}</span> certification.
              </p>
            </div>
            ...
            <div className="bg-white border border-slate-200 rounded-2xl p-5 shadow-sm space-y-3">
              ...
              <button onClick={() => navigator.share?.({ title: course.title, url: window.location.href })} className="w-full flex items-center justify-center gap-2 px-4 py-3 rounded-xl border border-slate-200 text-slate-600 font-semibold text-sm hover:bg-slate-50 transition-all border-dashed">
